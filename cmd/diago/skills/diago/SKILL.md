...
Runs native AST checks for cyclomatic complexity, function length, error handling, resource handling (unclosed resources), context use, dead-code hints, size checks, and maintainability smells. `audit` is the default, so `diago` with no subcommand runs it. Generated files are skipped across all checks since they can only be changed via codegen config. A file counts as generated by its name (`*.gen.go`, `*.sql.go`, `*.generated.*`) or a header comment (before the `package` clause) containing `DO NOT EDIT`, `Code generated`, or `generated by`. Pass `-include-generated` to audit them anyway. To skip any other file, add `//diago:ignore` in its header (optionally with a reason); ignored files are always skipped, even with `-include-generated`.
