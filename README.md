...
Generated files are skipped across all checks — AST, `-modernize`, `-u1000`, and `-infertypeargs` — since they can only be changed via codegen config, not hand edits. A file counts as generated by its name (`*.gen.go`, `*.sql.go`, `*.generated.*`) or by a header comment (before the `package` clause) containing `DO NOT EDIT`, `Code generated`, or `generated by` — which covers the canonical Go marker, swaggo headers, and looser variants. Pass `-include-generated` to audit them anyway.
