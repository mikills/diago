...
diago -target ./... -race -coverage -deps -u1000 -staticcheck # 5. tighten with deeper checks
...
Build the iterator checks as a standard `go vet` tool when editor or build-system integration is useful:
...
The vettool reports the same `sql-rows-err` and `scanner-err` categories as Diago’s native audit. It skips generated and whole-file-ignored sources, matching the default audit behavior.

JSON audits include the target module's declared Go version and toolchain, the running Go tool version, and rule metadata. `-modernize`, `-u1000`, `-staticcheck`, and `-infertypeargs` record their pinned tool version in check output. They may need Go's automatic toolchain selection or a compatible local toolchain.

#### Generated and ignored files

Generated files are skipped by AST, `-modernize`, `-u1000`, `-staticcheck`, and `-infertypeargs` checks because they must be changed through their generator configuration, not by hand. A file is generated when either condition holds:
